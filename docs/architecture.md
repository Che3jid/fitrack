...
食物库作为随网页和 iPhone App 打包的只读目录，保留 USDA FDC ID 与每 100 g 的热量、宏量营养素和钠。`domain/recipe.ts` 是纯计算模块：原料及调味料按重量加总，实际留在成品的油单独计入，成品重量只用于换算每 100 g 和食用份；烹饪方法仅提供操作提示，不对热量套用不可靠的通用系数。保存的食物记录包含本次食用份营养及可选钠值，旧记录无需数据迁移。
