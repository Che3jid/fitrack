...
export function lineChart(title: string, dates: string[], series: ChartSeries[], unit: string, includeZero = false): string {
  ...
  const grid = [0, 0.5, 1].map((ratio) => {
    ...
    return `<line x1="${left}" y1="${y(value)}" x2="${right}" y2="${y(value)}" stroke="#b4b8b1"/><text x="${left - 9}" y="${y(value) + 4}" text-anchor="end">${decimal(value)}</text>`;
  }).join('');
  ...
  const zero = min < 0 && max > 0 ? `<line x1="${left}" x2="${right}" y1="${y(0)}" y2="${y(0)}" stroke="#717b70" stroke-dasharray="3 4"/>` : '';
