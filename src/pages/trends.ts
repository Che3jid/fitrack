...
export function trendsPage(rows: TrendDay[], endDate: string, days: number, today: string): string {
  ...
  return `<section class="page-heading"><p class="eyebrow">趋势 · 把目光放长一点</p><h1 tabindex="-1">看见每一步改变。</h1><p class="muted">观察长期变化，不被单日波动定义。</p></section>
    ...
    <section class="card chart-card"><h2>体重趋势 <small>kg</small></h2>${lineChart('体重', dates, [
      { label: '每日体重', color: '#10b981', values: rows.map((day) => day.weight) },
      { label: '7 日平均', color: '#f59e0b', dashed: true, values: rows.map((day) => day.weightAverage.average) },
    ...
    <section class="card chart-card"><h2>摄入趋势 <small>kcal</small></h2>${lineChart('摄入', dates, [
      { label: '每日摄入', color: '#10b981', values: rows.map((day) => day.intake), provisional: rows.map((day) => !day.completed) },
      { label: '7 日平均', color: '#f59e0b', dashed: true, values: rows.map((day) => day.intakeAverage.average) },
    ...
    <section class="card chart-card"><h2>热量差趋势 <small>kcal</small></h2>${lineChart('热量差', dates, [
      { label: '摄入 − 当日 TDEE', color: '#60a5fa', values: rows.map((day) => day.balance), provisional: rows.map((day) => !day.completed) },
